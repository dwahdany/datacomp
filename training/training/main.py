...
def main(args, just_give_data: bool = False):
    args = parse_args(args)
    if args.indistribution_data_tar == "None":
        args.indistribution_data_tar = None
    ...
    args.tensorboard = (
        "tensorboard" in args.report_to or "all" in args.report_to
    )
    args.checkpoint_path = os.path.join(log_base_path, "checkpoints")
    if is_master(args):
        args.tensorboard_path = (
            os.path.join(log_base_path, "tensorboard")
            if args.tensorboard
            else ""
        )
    ...
    args.save_logs = (
        args.logs and args.logs.lower() != "none" and is_master(args)
    )
    writer = None
